...
Antes de propor qualquer linha de código para correções ou novos módulos, você deve **obrigatoriamente** absorver a memória de trabalho do projeto. Esta fase iniciará sempre que eu enviar o comando "/rag", lendo os seguintes arquivos em ordem:

1. `Engenharia/PROJECT_DNA.md` (Para restrições invioláveis, stack e regras de segurança).
2. `Engenharia/roadmap.md` (Para entender o roadmap de evolução do produto).
3. `Engenharia/lessons_learned.md` (Para evitar a repetição de bugs conhecidos).
4. `Engenharia/boas_praticas.md` (Para seguir os padrões estruturais da base de código).
5. `Engenharia/PRODUCT_BACKLOG.md` (Para entender o backlog de desenvolvimento).
6. `Engenharia/AI_EXECUTION_GUIDE.md` (Para entender como proceder com o desenvolvimento).
7. `Engenharia/PROJECT_DNA.md`(Para entender a regra de negócio do sistema)

*Assuma que o código de produção confia nestes artefatos. Violá-los significa quebrar a aplicação.*

Em seguida, aguarde a demanda para o desenvolvimento do hotfix ou do novo módulo.
...
3. **Validação RAG:** Esta alteração conflita com alguma restrição do `PROJECT_DNA.md` ou toca em alguma ferida já registrada no `lessons_learned.md`?
...
1. **Se foi um Hotfix (Bug):** Forneça uma nova entrada no formato exato do `lessons_learned.md` com a taxonomia correta (ex: `[BUG]`, `[SEC]`).
2. **Se foi um Novo Módulo:** Forneça a atualização para o `roadmap.md` e, se necessário, novas anotações de infraestrutura para o `README.md`.
3. **Se um novo padrão foi criado:** Forneça a atualização para o `boas_praticas.md`.
